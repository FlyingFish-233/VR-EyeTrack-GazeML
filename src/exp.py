...
# gpu是否可用
def gpu_availability(session_config):
    try:
...
# 初始化data_source和model
def init(args, session, gpu_available, batch_size):
...
def inference_loop(model, data_source, inferred_stuff_queue, visualize_thread):
...
if __name__ == '__main__':
    ...
    gpu_available = gpu_availability(session_config)
    ...
    with tf.Session(config=session_config) as session:
        ...
        # 初始化data_source和model
        data_source, model = init(args, session, gpu_available, batch_size)
        ...
        def _visualize_output():
            ...
            while True:
                ...
                for j in range(batch_size):
                    ...
                    eye_image = eye['image']
                    eye_side = eye['side'] # 左眼还是右眼
                    eye_landmarks = output['landmarks'][j, :] # 眼部关键点
                    eye_radius = output['radius'][j][0] # 虹膜半径
                    if eye_side == 'left': # 水平翻转左眼图像
                    ...
                    eyelid_landmarks = eye_landmarks[0:8, :] # 眼脸关键点
                    iris_landmarks = eye_landmarks[8:16, :] # 虹膜关键点
                    iris_centre = eye_landmarks[16, :] # 虹膜中心
                    eyeball_centre = eye_landmarks[17, :] # 眼球中心
                    eyeball_radius = np.linalg.norm(eye_landmarks[18, :] -
                                                    eye_landmarks[17, :]) # 眼球半径
        ...
        inference_loop(model, data_source, inferred_stuff_queue, visualize_thread)
